...
You describe a task you're considering using an LLM for. Cassandra then walks you through a series of questions, between 1 and 9 depending on the path:
...
3. **Three universal questions:** Consequence of error (what happens if nobody catches it?), data currency (is the data current, and would you know if it wasn't?), and judgement complexity (does the task need expertise the LLM lacks?). These are asked for every task and can escalate the verdict.
4. **What kind of task is this?** Deterministic, labelling, analysis, or generation. Each has different verification questions.
5. **Can you verify the output?** The specific questions depend on the task type.
...
This tells Cassandra: it's a deterministic task (code generation), it's testable, and there's security review. Clear path.
...
This tells Cassandra: it's an analysis task (summarisation from source material), with human checking, likely recurring. It will flag that the human checking will fade over time.
...
| "We have automated tests" | Makes deterministic code generation defensible |
| "We run automated security scanning" | Closes the security gap for code |
| "A script compares output to source data" | Provides faithfulness checking for analysis (summarisation) or structural verification for deterministic extraction |
...
+-- Yes -> How is the LLM used?
    |
    + Three universal questions (asked for every task):
    |   Consequence of error: contained / consequential / irreversible
    |   Data currency: current / mixed / unknown
    |   Judgement complexity: low / medium / high
    |   (Irreversible consequence or high judgement complexity
    |    escalates any CAUTION verdict to DON'T USE)
    |
    ...
    +-- Producing output -> What kind of task?
        +-- Deterministic -> Is this code generation?
        |   +-- Yes -> Can write automated tests?
        |   |   +-- No -> DON'T USE
        |   |   +-- Yes -> Security review?
        ...
        |   +-- No (extraction, retrieval, calculation) -> Can check output against source?
        |       +-- No -> Reclassify as analysis
        |       +-- Yes -> Automated or human checks?
        |           +-- Automated -> GO AHEAD
        |           +-- Human -> Repeating?
        |               +-- One-off -> GO AHEAD (check this run)
        ...
        |   +-- No -> DON'T USE
        |   +-- Yes -> Same LLM or separate process?
        |       +-- Same LLM -> DON'T USE (self-verification is not a check)
        |       +-- Separate process -> Automated or human?
        |           +-- Automated -> GO AHEAD
        |           +-- Human -> Repeating?
        |               +-- One-off -> CAUTION (verify this one)
        |               +-- Recurring -> DON'T USE (checking will fade)
        +-- Generation -> Are there clear criteria to check against?
            +-- No -> DON'T USE
            +-- Yes -> Automated or human checks?
                +-- Automated -> GO AHEAD
                +-- Human -> Repeating?
