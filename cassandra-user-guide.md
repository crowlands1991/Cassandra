...
Does accuracy matter? If not, you're clear to use an LLM freely.
Is the LLM producing the output itself, or using tools? An LLM generating a report is different from an LLM that writes a database query and summarises the results. Each has its own path.
What kind of task is this? Deterministic, labelling, analysis, or generation. Each has different verification questions.
Can you verify the output? The specific questions depend on the task type.
